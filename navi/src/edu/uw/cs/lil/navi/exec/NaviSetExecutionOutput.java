...
public class NaviSetExecutionOutput implements
	...
	public NaviSetExecutionOutput(
			...
			final IJointModelImmutable<Instruction, LogicalExpression, Trace> model,
			long execTime) {
	...
	public List<IExecution<List<Pair<LogicalExpression, Trace>>>> getExecutions(
			IFilter<List<Pair<LogicalExpression, Trace>>> filter) {
		...
		for (final IExecution<List<Pair<LogicalExpression, Trace>>> execution : allExecutions) {
			if (filter.isValid(execution.getResult())) {
				execs.add(execution);
			}
		}
		return execs;
	...
	@Override
	public List<IExecution<List<Pair<LogicalExpression, Trace>>>> getExecutions(
			final List<Pair<LogicalExpression, Trace>> label) {
		return getExecutions(new IFilter<List<Pair<LogicalExpression, Trace>>>() {
			
			@Override
			public boolean isValid(List<Pair<LogicalExpression, Trace>> e) {
